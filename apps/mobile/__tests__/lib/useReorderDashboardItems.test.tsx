...
const sampleFavorites: DashboardItem[] = [
...
describe('useReorderDashboardItems (mobile)', () => {
  ...
  it('optimistically reorders the local store with dense positions', () => {
    ...
    useDashboardStore.getState().hydrate(sampleFavorites)

    const { result } = renderHook(() => useReorderDashboardItems(), { wrapper })
    const newOrder: DashboardItem[] = [sampleFavorites[2], sampleFavorites[0], sampleFavorites[1]]
