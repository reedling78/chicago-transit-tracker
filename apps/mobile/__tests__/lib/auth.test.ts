...
describe('signInWithApple', () => {
  ...
  describe('Android', () => {
    ...
    it('opens the in-app Apple OAuth flow via the web bridge and signs into Firebase', async () => {
      ...
      // Firebase credential built from the id_token returned in the deep-link fragment
      const providerInstance = mockOAuthProvider.mock.results[0].value as {
        credential: jest.Mock
      }
      expect(providerInstance.credential).toHaveBeenCalledWith({
    ...
    it('throws when the user cancels the OAuth session', async () => {
      mockOpenAuthSessionAsync.mockResolvedValueOnce({ type: 'cancel' })
      await expect(signInWithApple()).rejects.toThrow(/cancelled or failed/i)
    ...
    it('throws when Apple returns no id_token', async () => {
      mockOpenAuthSessionAsync.mockResolvedValueOnce({
        type: 'success',
        url: 'ctt://apple-callback#code=just-a-code&state=anything',
      })
      await expect(signInWithApple()).rejects.toThrow(/did not return an id_token/i)
      expect(mockSignInWithCredential).not.toHaveBeenCalled()
    })

    it('throws when the returned state does not match what was sent', async () => {
      mockOpenAuthSessionAsync.mockResolvedValueOnce({
        type: 'success',
