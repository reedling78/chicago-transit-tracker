...
interface DashboardItemMenuProps {
  item: DashboardItem
  lines: Line[] | undefined
  stations: Station[] | undefined
  /** Schedule for the station item (if any). Used to populate CTA headsign chips. */
  ...
  /**
   * Train items only. Invoked when the user picks "Set departure station…"
...
export default function DashboardItemMenu({
  ...
  // menu immediately (the `item` prop is a snapshot from menu-open time).
  const liveItem = useDashboardStore((s) =>
    s.items.find((i) => i.type === item.type && i.id === item.id),
  )
  ...
  return (
    <div
      ref={ref}
      role="menu"
      aria-label="Dashboard item actions"
