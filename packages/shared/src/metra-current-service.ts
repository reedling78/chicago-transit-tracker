/**
 * Pure helpers for the "Current service" line view, shared by web and mobile.
 *
 * None of these functions depend on React, the DOM, or any platform API — the
 * MetraCurrentService component on each platform is a thin shell that calls the
 * platform's `useMetraFeed` hook and then hands its data to the functions here.
 */
...
import type { ServiceType } from './gtfs-types'

/** One Metra trip (a single train number on a line) with its full stop sequence. */
...
/** One presentational row in the Current service list. */
